...
class GameClassicAPIView(APIView, CustomPagination):
    ...
    def get(self, request):
        if request.user.is_anonymous:
            ...
            if game_type:
                if game_type == 'single':
                    games.filter(category='Classic')
                elif game_type == 'group':
                    games.filter(category='Event')
            ...
            sort = request.query_params.get('sort', None)
            if sort == 'new':
                games = Game.objects.filter(Q(category='Classic') & Q(status=True)).order_by('-created_at')
            elif sort == 'easy':
                ...
                # games = Game.objects.extra(select={'difficulty_order': CASE_SQL}, order_by=['difficulty_order'])
                games = []
                games_easy = Game.objects.filter(Q(category='Classic') & Q(status=True) & Q(difficulty='easy'))
                games_medium = Game.objects.filter(Q(category='Classic') & Q(status=True) & Q(difficulty='medium'))
                games_hard = Game.objects.filter(Q(category='Classic') & Q(status=True) & Q(difficulty='hard'))
                for game in games_easy:
                    games.append(game)
                for game in games_medium:
                    games.append(game)
                ...
                games = Game.objects.filter(Q(category='Classic') & Q(status=True)).order_by('-total_point')
            else:
                ...
                cursor.execute("""SELECT * FROM (
                        ...
                        WHERE distance <= 10000 AND category='Classic' AND status=1
                        ORDER BY distance;""".format(user.profile.latitude, user.profile.longitude))
                rows = cursor.fetchall()
                games = []
                for row in rows:
                    game = Game.objects.get(pk=int(row[0]))
                    games.append(game)
            data = self.paginate_queryset(games, request, view=self)
            serializer = GameSerializer(data, many=True)
            return self.get_paginated_response(serializer.data)
